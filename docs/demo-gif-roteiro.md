...
**Objetivo:** GIF mudo, ~20-25s, loop, que cabe no placeholder da landing (`.kindraw-landing__demo`, frame 16:10). **História (a da pesquisa de GTM):** prompt no Claude Code → diagrama com layout de verdade → abre o share link → edita junto. O ponto não é "o agente desenha", é **"o diagrama fica vivo num workspace compartilhável"**. **Cenário:** serviço de pagamentos (bate com a landing `payments-architecture` e o copy do step 2).
